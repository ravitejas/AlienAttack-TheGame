...
function Start () {
	...
	xDistance = screenWidth/8 - 60;
	yDistance = screenHeight/10 + 40;
	
	if(!PlayerPrefs.HasKey("toggleMusic"))
	{
		//The key is not in player preference. Add it.
...
function OnGUI () {
	...
	{	
		...
		modalAlertWindowRect = GUI.ModalWindow (0, Rect (175,125,550, 250), resetGame, alertWindowTitleContent, buttonResetAlertGameGUI);
		alertWindowTitleContent.image = icon;
		buttonResetAlertGameGUI.normal.textColor = Color.white;
	}
	else
	{
	...
	if (GUI.Button (Rect(750, 475, buttonHeight, buttonWidth), "BACK", buttonBackGUI))
	{
		//Back to Main Screen
		PlayerPrefs.Save();
